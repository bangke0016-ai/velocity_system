...
<div class="chat-admin" data-admin-chat-list data-endpoint="admin-chat-api.php">
  <div class="panel chat-list">
    ...
    <div class="chat-list-items">
      ...
      <?php foreach ($chatConversations as $conversation): ?>
        <a class="chat-list-item <?= $selectedConversation === $conversation['conversation_token'] ? 'active' : '' ?>" data-last-message-id="<?= $e($conversation['last_id']) ?>" href="admin.php?tab=chat&amp;conversation=<?= $e($conversation['conversation_token']) ?>">
  ...
  <div class="panel chat-room">
    ...
    <?php else: ?><div class="chat-room-body" data-admin-chat-room data-conversation="<?= $e($selectedConversation) ?>" data-endpoint="admin-chat-api.php"><div class="chat-thread">
      <?php foreach ($chatMessages as $message): ?><div class="chat-bubble <?= $message['sender'] === 'admin' ? 'admin' : ($message['sender'] === 'assistant' ? 'assistant' : 'visitor') ?>" data-message-id="<?= $e($message['id']) ?>"><?= $e($message['message']) ?><small><?= $message['sender'] === 'admin' ? 'Admin' : ($message['sender'] === 'assistant' ? 'Velocity Assistant' : 'Pengunjung') ?> · <?= $e(date('d M Y, H:i', strtotime($message['created_at']))) ?></small></div><?php endforeach; ?>
...
<?php if (!$selectedConversation): ?><script>
(function () {
  const list = document.querySelector('[data-admin-chat-list]');
  if (!list) return;
  const endpoint = list.dataset.endpoint;
  const currentCount = list.querySelectorAll('.chat-list-item').length;
  const currentLatestId = Math.max.apply(null, Array.from(list.querySelectorAll('[data-last-message-id]')).map(function (item) { return Number(item.dataset.lastMessageId || 0); }).concat(0));
  window.setInterval(async function () {
    try {
      const response = await fetch(endpoint, {headers: {'Accept': 'application/json'}, cache: 'no-store'});
      if (!response.ok) return;
      const data = await response.json();
      const latestId = data.conversations ? Math.max.apply(null, data.conversations.map(function (item) { return Number(item.last_id || 0); }).concat(0)) : 0;
      if (data.success && data.conversations && (data.conversations.length !== currentCount || latestId !== currentLatestId)) window.location.reload();
    } catch (error) {
      return;
    }
  }, 5000);
...
<?php if ($selectedConversation): ?><script>
(function () {
  const room = document.querySelector('[data-admin-chat-room]');
  if (!room) return;
  const thread = room.querySelector('.chat-thread');
  const conversation = room.dataset.conversation;
  const endpoint = room.dataset.endpoint;
  let lastMessageId = 0;
  let loading = false;

  thread.querySelectorAll('[data-message-id]').forEach(function (message) {
    lastMessageId = Math.max(lastMessageId, Number(message.dataset.messageId));
  });
  ...
  function appendMessages(messages) {
    messages.forEach(function (message) {
      ...
      const sender = message.sender === 'admin' ? 'admin' : (message.sender === 'assistant' ? 'assistant' : 'visitor');
      bubble.className = 'chat-bubble ' + sender;
      bubble.dataset.messageId = message.id;
      bubble.textContent = message.message;
      const meta = document.createElement('small');
      meta.textContent = sender === 'admin' ? 'Admin' : (sender === 'assistant' ? 'Velocity Assistant' : 'Pengunjung');
  ...
  async function pollMessages() {
    ...
    loading = true;
    try {
      const response = await fetch(endpoint + '?conversation=' + encodeURIComponent(conversation) + '&after=' + lastMessageId, {headers: {'Accept': 'application/json'}, cache: 'no-store'});
      if (!response.ok) return;
      const data = await response.json();
